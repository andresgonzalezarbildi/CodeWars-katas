...
const getPositions = s => {
  //will the input always be a number, no string, can it be 0? 
  //are there any edge cases i should know about
  
  
  //i should return an array with the numbers of hand or each person
  ...
  //declare a new array
  let result = []
  //decompose the number in factors of 3
  
  //push %3 for the first one
    result.push(s % 3)
  // push floor / 3 %3 for the second one
  result.push(~~(s/3) % 3)
  //push floor / 9 % 3 for the third one
    result.push(~~(s/9) %3)
  //return result array
  return result
}
